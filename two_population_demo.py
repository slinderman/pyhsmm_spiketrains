...
T = 1000        # Number of time bins to simulate per trial
...
S_train, _ = true_hmm.generate(T, keep=True)
S1_train, S2_train = S_train[:, :N1].copy('C'), S_train[N1:].copy('C')
S_test, _ = true_hmm.generate(T, keep=True)
S1_test, S2_test = S_test[:, :N1].copy('C'), S_test[N1:].copy('C')

# Mask off the activity from population 2
mask1 = np.hstack([np.ones((T, N1), dtype=bool), np.zeros((T, N2), dtype=bool)])

# Compute true likelihoods
ll_S1tr_true = true_hmm.log_likelihood(S_train, mask=mask1)
ll_S1te_true = true_hmm.log_likelihood(S_test, mask=mask1)
ll_S2te_given_S1te_true = true_hmm.predictive_log_likelihood(S_test, mask=mask1)
...
ll_S1tr_aug = aug_hmm.log_likelihood(S_train, mask=mask1)
ll_S1te_aug = aug_hmm.log_likelihood(S_test, mask=mask1)
...
ll_S2te_given_S1te_aug = aug_hmm.predictive_log_likelihood(S_test, mask=mask1)
